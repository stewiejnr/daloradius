# Un-official daloRADIUS Dockerfile
# GitHub: https://github.com/stewiejnr/daloradius
#
# Build image:
# 1. git pull git@github.com:stewiejnr/daloradius.git
# 2. docker build . -t stewiejnr/daloradius
#
# Run the container:
# 1. docker run -p 80:80 -d stewiejnr/daloradius

FROM ubuntu:20.04

LABEL Description="daloRADIUS Un-official Docker based on Ubuntu 20.04 LTS and PHP7." \
	License="GPLv2" \
	Usage="docker build . -t stewiejnr/daloradius && docker run -d -p 80:80 stewiejnr/daloradius" \
	Version="1.0.0"
